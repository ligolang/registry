...
let routes = (addr: any) => {
  let fetch =
    typeof window === 'undefined'
      ? (path: string) => {
          if (addr) {
            return nodeFetch(`http://${addr.host}:${addr.port}${path}`);
          } else {
            throw new Error('Did not expect addr to be void');
          }
        }
      : window.fetch;
...
export function init(addr: any) {
  return new UniversalRouter(routes(addr));
}

export function resolve(router: any, path: any): Promise<any> {
