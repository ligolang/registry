...
import ligoWebEndpointsApi from '../ligo-api';
...
export function ligoWebAPI(config: Config, auth: IAuth, storage: Storage): Router {
  // eslint-disable-next-line new-cap
  const route = Router();
  // validate all of these params as a package name
  // this might be too harsh, so ask if it causes trouble=
  ...
  route.use(auth.webUIJWTmiddleware());
  route.use(setSecurityWebHeaders);
  route.use(ligoWebEndpointsApi(auth, storage, config));
  return route;
}
